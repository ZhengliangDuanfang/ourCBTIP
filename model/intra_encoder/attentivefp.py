...
# 文档见 https://lifesci.dgl.ai/api/model.gnn.html#module-dgllife.model.gnn.attentivefp
# 源代码位于 https://github.com/awslabs/dgl-lifesci/blob/master/python/dgllife/model/gnn/attentivefp.py
# 除部分函数不同（可能是dgl.function版本不同导致的）外，无实质性区别
...
class AttentiveGRU2(nn.Module):
    ...
    def forward(self, g, edge_logits, node_feats):
        ...
        """
        g = g.local_var()
        g.edata['a'] = edge_softmax(g, edge_logits) # att
        g.ndata['hv'] = self.project_node(node_feats)

        g.update_all(fn.src_mul_edge('hv', 'a', 'm'), fn.sum('m', 'c'))
        context = F.elu(g.ndata['c']) # Msg
        return F.relu(self.gru(context, node_feats)) # Update Feature
...
class GNNLayer(nn.Module):
    ...
    def forward(self, g, node_feats):
        ...
        g = g.local_var()
        g.ndata['hv'] = node_feats
        g.apply_edges(self.apply_edges) # 向量拼接 
        logits = self.project_edge(g.edata['he']) # 生成边的 logits

        return self.attentive_gru(g, logits, node_feats) # 生成新 feature
